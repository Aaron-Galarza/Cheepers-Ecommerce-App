...
export const usePromoManagement = () => {
  ...
  const fetchPromotions = useCallback(async () => {
    try {
      ...
      const filteredPromos = response.data.filter(p => isPromoCategory(p.category));
      setPromotions(filteredPromos);
    } catch (err: any) {
      console.error('Error al cargar las promociones:', err);
      if (axios.isAxiosError(err) && err.response && err.response.status === 401) {
        authService.logout();
        setError('Sesion expirada o no autorizada. Por favor, inicia sesion de nuevo.');
      } else {
        setError('No se pudieron cargar las promociones. Verifica la conexion o tu sesion.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleCreatePromotion = useCallback(async (data: NewPromotionData) => {
    try {
      ...
      console.error('Error al crear promocion:', err);
      if (axios.isAxiosError(err) && err.response && err.response.status === 401) {
        authService.logout();
        setError('Sesion expirada o no autorizada. Por favor, inicia sesion de nuevo.');
      } else {
        setError('Error al crear promocion. Verifica los datos y tu sesion.');
      }
  ...
  const handleUpdatePromotion = useCallback(async (id: string, data: Promotion) => {
    try {
      ...
      console.error('Error al actualizar promocion:', err);
      if (axios.isAxiosError(err) && err.response && err.response.status === 401) {
        authService.logout();
        setError('Sesion expirada o no autorizada. Por favor, inicia sesion de nuevo.');
      } else {
        setError('Error al actualizar promocion. Verifica los datos y tu sesion.');
      }
      return { success: false, error: err.response?.data?.message || err.message };
    }
  }, [fetchPromotions]);
  ...
  const handleDeletePromotion = useCallback(async (id: string) => {
    console.log('Confirmacion de eliminacion: Estas seguro de que quieres eliminar esta promocion?');
    try {
      const token = localStorage.getItem('adminToken');
      ...
      if (axios.isAxiosError(err) && err.response && err.response.status === 401) {
        authService.logout();
        setError('Error al eliminar promocion. Verifica tu sesion.');
      } else {
        setError('Error al eliminar promocion. Verifica tu sesion.');
      }
      return { success: false, error: err.response?.data?.message || err.message };
  ...
  const handleToggleActive = useCallback(async (id: string, currentStatus: boolean) => {
    try {
      ...
      console.log(`Promocion ${id} cambiada a ${response.data.product.isActive}`);
      fetchPromotions();
      return { success: true };
    } catch (err: any) {
      console.error(`Error al cambiar estado de promocion ${id}:`, err);
      if (axios.isAxiosError(err) && err.response && err.response.status === 401) {
        authService.logout();
        setError('Sesion expirada o no autorizada. Por favor, inicia sesion de nuevo.');
      } else {
        setError('Error al cambiar estado de promocion. Estas logueado?');
      }
      return { success: false, error: err.response?.data?.message || err.message };
    }
  }, [fetchPromotions]);
