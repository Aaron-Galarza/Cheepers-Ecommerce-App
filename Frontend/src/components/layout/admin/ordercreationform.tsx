...
const OrderCreationForm: React.FC<OrderCreationFormProps> = ({
  ...
  const handleAddProduct = () => {
    setNewOrderForm(prev => ({
      ...prev,
      formProducts: [...prev.formProducts, { tempId: Date.now().toString(), productId: '', quantity: 1, selectedAddOns: [] }],
    }));
  };
    ...
    setNewOrderForm(prev => ({
      ...
      formProducts: prev.formProducts.map(fp =>
        fp.tempId === productTempId
          ? {
              ...fp,
              selectedAddOns: fp.selectedAddOns.map((sa, idx) =>
                idx === addOnIndex ? { ...sa, [field]: value } : sa
              ),
            }
          : fp
      ),
          ...
          {newOrderForm.formProducts.map((fp) => (
            <div key={fp.tempId} className={styles.productItem}>
              ...
              <input
                ...
                required
              />
              <button type="button" onClick={() => handleAddAddOn(fp.tempId)} className={styles.addAddOnButton}>
                <FaPlus /> Adicional
              </button>
              {newOrderForm.formProducts.length > 1 && (
                <button type="button" onClick={() => handleRemoveProduct(fp.tempId)} className={styles.removeButton}>
                  <FaMinus />
                </button>
              )}
              ...
              {fp.selectedAddOns.length > 0 && (
                <div className={styles.addOnsContainer}>
                  <h4>Adicionales para {productMapRef.current.get(fp.productId) || 'este producto'}:</h4>
                    ...
                    <div key={addOnIndex} className={styles.addOnItemForm}>
                      ...
                      >
                        ...
                        {availableAddOns.map(ao => (
                          <option key={ao._id} value={ao._id}>{ao.name} (${ao.price.toFixed(2)})</option>
                        ))}
                      </select>
                      <input
                        type="number"
                        placeholder="Cantidad"
                        value={sa.quantity}
                        onChange={(e) => handleAddOnChange(fp.tempId, addOnIndex, 'quantity', parseInt(e.target.value) || 1)}
                        className={styles.inputField}
                        min="1"
                        required
                      />
                      <button type="button" onClick={() => handleRemoveAddOn(fp.tempId, addOnIndex)} className={styles.removeButton}>
                        <FaMinus />
                      </button>
                    </div>
                  ))}
